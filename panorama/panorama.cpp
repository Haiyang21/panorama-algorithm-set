...
void warpAndFuseImage(const Mat &img1, const Mat &img2, const Matrix3f &H, int &offX, int &offY, int &cw, int &ch) {
	...
	Mat linearFusion = Mat::zeros(ch, cw, CV_8UC3); img1.copyTo(warp_img1(Rect_<int>(-minX, -minY, width, height))); uchar r, g, b;
	float h00 = H(0, 0);
	float h01 = H(0, 1);
	float h02 = H(0, 2);
	float h10 = H(1, 0);
	...
	for (int j = 0; j < cw; j++) {
		...
		if (leftWeight + rightWeight != 0) {
			r = static_cast<uchar>(img1Color[0] * leftWeight / (leftWeight + rightWeight) + img2Color[0] * rightWeight / (leftWeight + rightWeight));
			g = static_cast<uchar>(img1Color[1] * leftWeight / (leftWeight + rightWeight) + img2Color[1] * rightWeight / (leftWeight + rightWeight));
			b = static_cast<uchar>(img1Color[2] * leftWeight / (leftWeight + rightWeight) + img2Color[2] * rightWeight / (leftWeight + rightWeight));
	...
	if (saveData)
		imwrite("Global.jpg", linearFusion);
}
...
{
	const char *img1_path = "pan1.jpg";
	const char *img2_path = "pan2.jpg";
	//const char *img1_path = "4.JPG";
	//const char *img2_path = "5.JPG";
	MatrixXf inlier, A;
	Matrix3f T1, T2;
	Mat img1, img2;
	int offX, offY, cw, ch;
